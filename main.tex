...
\usepackage{pdfpages} % To include article manuscripa}
...
This thesis discusses the development and application of comprehensively coupled
(supercritical fluid × gas) chromatography to the chemical analysis of biodiesel and
biodiesel blends. In a future low-carbon economy there might still be a need for
large, highly-efficient diesel engines, which can be fuelled with a high-quality
carbon-neutral fuel, such as biodiesel. The quality of such fuels are judged
according to technical standards, documents that detail the requirements of
compliance. Liquid fuels are complex mixtures that offer challenges to
separation science. Chromatographic separations that use different separation
mechanisms can be combined to meet those challenges, culminating in
comprehensive coupling, where every fraction of a first separation is subjected
to a second separation. The difference between the separations is known as
orthogonality, and the higher the orthogonality the more useful the
comprehensive coupling. The orthogonality between supercritical fluid
chromatography (SFC) and gas chromatography (GC) is very high, and their
coupling is technically simple. But to make the coupling practical, the GC
separation must be fast and temperature-programmed.  The construction of a
resistive coaxial heater with active cooling by liquid carbon dioxide allows the
recording of hundreds of consecutive fast temperature programmed chromatograms.
When these chromatograms are of GC separations of fractions from an
SFC separation they can be combined to construct a two-dimensional SFC×GC
chromatogram. When biodiesel and blends of diesel and biodiesel are analysed by
SFC×GC, separation in the first dimension is by polarity and degree of
unsaturation, and in the second dimension by volatility. The resulting
chromatograms contain powerful patterns of peaks, clearly separating the
aromatic hydrocarbons from the alkanes and alkenes, and the fatty acid methyl
esters (FAMEs) of the biodiesel from the hydrocarbons of the petrodiesel.
Because the flame ionization detector (FID) is used, quantification is
straightforward and reliable. The FID remains compatible with SFC×GC even when
organic modifiers are added to the SFC mobile phase because they elute on the GC
as a solvent peak, separate from the analytes of interest. SFC×GC can be used
for research and quality control in the liquid fuels or vegetable oil
industries.
...
\include{Appendices/AppendixA} % Index
\include{Appendices/AppendixB} % Published paper
%\include{Appendices/AppendixC} 
\include{Appendices/AppendixD} % Engineering drawings
\include{Appendices/AppendixE} % Fatty acid profiles
