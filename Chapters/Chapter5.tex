...
\ref{fig:trilemma}): Any chromatographic method that involves decisions
...
run a faster chromatogram with higher resolution. Any attempt to increase the
...
have fewer plates, and hence decreased resolution. This is easily seen in the
resolution equation \autocite{Sandra1989}
...
\( H \) depends on \( \bar{u} \) in the \textit{Van Deemter Equation} \( \hat{H}
= \frac{B}{\bar{u}} + {C}{\bar{u}} \) or under conditions of high pressure drop,
the equivalent equation by Blumberg \autocite{Blumberg1997} \( H =
\frac{B}{\bar{u}^2} + C_1\bar{u}^2 + C_2\bar{u}\). These equations do have
minima, but under most experimental conditions the flow will be at such a rate
that decreasing the flow will decrease the plate height. Hence, increasing the
flow while keeping the same column length will decrease the resolution \( R_s \).
...
chromatography using shorter columns, higher flow rates, temperature
...
\textit{resistive heating}\footnote{Alternative methods of heating by electromagnetic fields are \textit{inductive
...
field. But the electrons are within the crystal lattice of the metal, where their
...
and the ratio $\frac{V}{I}$ defines a proportionality constant $R$, called the
...
Applying a voltage \(V\) to a metal element in close proximity to the column
will heat the metal, and therefore the column in contact with it. If the volume
of the metal is small enough, and the current high enough, the temperature of
the metal element will increase, and with it the temperature of the column. The
rate at which the piece of metal heats up depends on the power dissipated, the
mass of the metal, and its heat capacity. By suitable manipulation of \(V\),
then, the temperature of the column can then be changed at any desirable rate.
...
As an example of collinear resistive heating stands Agilent\texttrademark{}'s
...
It had an outside diameter of 1.06 mm and an inside diameter of 0.80 mm. The
column is threaded inside the stainless steel tube, so that it is in close
contact with the heater, giving reliable heat transfer.  The coaxial heater can be
rolled up to fit inside a conventional GC oven.

This coaxial design has the advantage that the column can be changed without
changing the resistive heater. This means that it can be simultaneously used as
a sensing element that reports the average temperature of the tube, and thereby
...
\autocite{Dyos2012}. But a fairly simple electronic circuit can be used to
...
therefore it is amplified by the amplifier with gain $g$, so that $V_b =
gV_{ref}$. $V_b$ is measured, as is $dV$, the potential difference between the
supply and the amplifier output.
...
Thermocouple is well established and is widely used in industry for measuring
temperatures. Thermocouples don't have to be calibrated. They are usually
constructred from well-characterized alloys that generate predictable voltages
for given junction temperatures. Thermocouple alloys that have been standardized
are known as 'types'. We selected the general-purpose Type K thermocouple.
...
part, allowing the molten metals first to mix, and then to solidify. This creates a
...
into an arc, the heat of the arc melted the end of the thermocouple. The molten
metal would then contract into a spherical globule, which grew as the arc added
more heat. As more of the metal of the wire melted, the globule would move away
from the carbon electrode, until the gap became too large to sustain the arc.
The current would then stop, leaving a spherical welded bead at the end of the
wires. The process could be repeated as often as necessary to obtain a bead of
the desired size. The steps of the process is shown in Figure
\ref{fig:WeldingSteps}. It is worth noting that it is absolutely necessary to
form an arc: if the carbon electrode happened to touch the wire so that a
current flowed directly from the wire to the carbon electrode the wire would
rapidly heat up over its length, and melt. It is also interesting to note that
it seemed necessary to have a roughly broken carbon electrode surface: a
polished surface would not generate an arc, or even make electrical contact with
the wires. This might be because the graphite used for the electrode was
formulated for use in pencils.
...
had to be inserted into the coaxial heater. For this we used fused silica
capillaries with an inside diameter of \SI{0.25}{\milli\metre} and an outside
diameter of \SI{0.4}{\milli\metre}. These were easily obtained in the form of
worn-out chromatographic columns. A narrower capillary was used to draw the wires
into the probe capillary, in a process described in Figure
...
is quite heavy, and has to transmit the forces of the coolant tube and the
...
electrical isolation needs to be maintained. The columns are also fragile, and
...
alignment of the T-piece blocks and the inlet/detector.
...
and at the top adjustable points pressed against pressure plates which pressed
...
a bank of six NPN transistors, connected in parallel to distribute the heat
dissipation. The final control signal was a voltage set either by a
