...
collected \autocite{Murphy1998}, but more is better. For example, if the peaks
...
the current problem, pick a value that is likely to be satisfactory for most
...
Any chromatographic optimization goal must contain resolution as part
...
flow rate (\(F\)). Traditionally \(\overline{u}\) has been used, because it can
...
It is clear that the function has a minimum. The flow rate at this minimum
...
with shorter columns gives better performance \autocite{Klee2002, Reed1999} than
flow much faster than SOF combined with longer columns.
...
their elution will be optimal. With a precise control system the temperature can
be continuously adjusted so that the selectivity conditions always match the
kinetic conditions, and all the compounds elute optimally. The technology for
\keyword{temperature programming} in GC is mature, and every column manufacturer
specifies temperature limits for isothermal and ramped temperature programs for
every stationary phase.
...
\autocite{Marriott2012}. In GC×GC the \twoD separation is isothermal, but because
...
the ramp rate needs to be thousands of \SI{}{\celsius\per\minute}, because the
suggested ramp rate is inversely proportional to the void time: as the void time
gets smaller the suggested ramp rate increases rapidly (See Figure \ref{fig:RampRateBlumberg}).
...
the inelastic collisions will increase the vibrational frequency of atoms, and
