...
the \keyword{void time}, which is easily determined by measuring the
...
\keyword{efficiency optimized velocity} (EOV) depends on the column length when
...
simple theoretical result is that under \keyword{speed-optimized flow} (SOF),
...
to relax the plate height constraint: \keyword{selective detection} and
\keyword{sample preparation}. In selective detection a detector is chosen that
only responds to the analytes, and not to any interfering compounds. For
example, an \keyword{electron capture detector} (ECD) responds strongly to
...
\keyword{solid phase extraction} (SPE) cartridge before it is injected into the
