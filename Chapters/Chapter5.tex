...
while subject to to some \keyword{constraints}. The amount of ``resources'' that
are used are the \keyword{optimizing parameters}.
...
parameters. \keyword{Selectivity optimization} is the process of selecting the
...
\keyword{Kinetic optimization} is the process of selecting the best combination
...
of the efficiency gives the \keyword{plate height} (\(H\)). Plate height is a
...
the \keyword{void time}, which can be conveniently determined by measuring the
...
When the amount of stationary phase is small, also known as the \keyword{thin-film case}, then \(c_2 = 0\) and \(C_2 = 0\), and
...
It is clear that the function has a minimum, and the flow at this minimum can be
called \(F_{Hmin}\). This flow is called efficiency-optimized flow, EOF.
