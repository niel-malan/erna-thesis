...
fractions of eluate. These devices became known as \keyword{modulators} and made
...
The rate at which fractions are collected from the \oneD separation is
...
however, \keyword{sample throughput} of a chromatographic method needs to be
acceptable. One of the major determinants of sample throughput is \keyword{run
time}, the time it takes from when the sample is injected until the the final
...
would take, \(t_m\) is the \keyword{modulation period}, and \(t_{stopped}\) is
...
Every chromatographer has met the chromatographer's trilemma (See Figure
\ref{fig:trilemma}): Any chromatographic method that involves decisions about
speed, resolution, and capacity can maximize only one at a time\footnote{This
trilemma applies only to capillary chromatography. When using packed columns,
the capacity can be readily increased by using a column with a larger diameter
and a larger amount of packing.}. The fastest chromatography will have low
capacity and low resolution, the chromatography giving the highest resolution
will be slow and have low capacity, and the chromatogram of a sample with a high
concentration of analyte will be slow and have low resolution
\autocite{Klee2002}. One way the chromatographer can ease the grip of the
trilemma is to decide what factor is least important for the current problem,
pick a value for that is likely to be satisfactory for most conditions, keep it
fixed, and then find an acceptable compromise between the other two. In the
development of the SFC×GC we decided to work with a fixed sample capacity: we
chose a capillary column with a \SI{0.25}{\milli\metre} internal diameter and a
\SI{0.25}{\micro\metre} stationary phase. 

With the sample capacity held constant, it is now possible to search for
compromise conditions that give adequate resolution in the shortest possible
time.
...
traditional but less intuitive \keyword{number of plates} \(N\) through the
expression \(N = E^2\).

The theory of fast chromatography has been developed in a series of papers by
Blumberg \autocite{Blumberg1997}.

\begin{todo}
The theory of fast gas chromatography has been well developed
\autocite{Blumberg1997}, and 
\end{todo}
...
%\todo{We avoided the trilemma by not bothering with optimizing column capacity. Sample
...
Where the \(N\) is the \keyword{number of plates}, \(\alpha\) is the
\keyword{selectivity}, and \(k_2\) is the \keyword{retention factor}.

\(N\) is can be calculated from \(N = \frac{L}{H} \), where \(L\) is the
\keyword{column length}, and \(H\) is the \keyword{plate height}.

Therefore \( R_s \propto \sqrt{\frac{L}{H}} \), and obviously reducing the
length of the column will reduce the resolution \(R_s\). 

\( H \) depends on gas flow, in a relationship described by the \keyword{Van
...
excess resolution is often termed \keyword{sample clean-up}. While usually
...
\todo{Chromatography theory only applies to isothermal/isocratic condition}

Temperature programming is, of course, not necessary only for fast
chromatography, but has long been implemented to avoid the \keyword{general
...
\keyword{Thermal imaging} is the process by which infrared radiation from objects
...
This examination of the uniformity of the coaxial heater helped to dispell fears
...
conductors used to generate a voltage is known as a \keyword{thermocouple}, and
...
'trapped'. This trapping, also called \keyword{cryo-trapping} or
\keyword{cryo-focusing} is useful in various aspects of gas chromatography, such
