...
The rate at which fractions of the \oneD are collected separation is determined
...
would take, \(P_M\) is the \keyword{modulation period}, and \(t_{stopped}\) is
...
of ``resources'' used are the \keyword{optimizing parameters}.

There are two kinds of optimization in chromatography: selectivity and kinetic.
\keyword{Selectivity optimization} is the process of selecting the best
...
Efficiency is related to the more traditional but less intuitive \keyword{number
of plates} \(N\) through the expression \(N = E^2\). The length of the column
(\(L\)), divided by the square of the efficiency gives the \keyword{plate
height} (\(H\)). Plate height is a measure of how much each unit of length of
...
two ways to relax the plate height constraint: \keyword{selective detection} and
\keyword{sample cleanup}. In selective detection a detector is chosen that only
responds to the analytes, and not to any interfering compounds. For example, an
\keyword{electron capture detector} (ECD) responds strongly to halides, so a
...
GC-MS using \keyword{single ion monitoring} (SIM) also enables faster
