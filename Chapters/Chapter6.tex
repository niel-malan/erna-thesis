...
but the method for  determining the \emph{total amount} of FAMEs is a
completely different method from that for determining the amount of
\emph{unsaturated} FAMEs. Both these methods generate complex chromatograms
that need highly skilled and experienced chromatographers to interpret. There is
no doubt that the use of artificial intelligence and other technological
innovations for interpreting chromatograms will grow, but the paradox of
...
by SFC in using neat carbon dioxide as a mobile phase and unmodified silica as
...
polyimide coating. Like all resins and polymers the polyimide coating degrade faster
at higher temperatures, so the longer the column spends at high temperature the
sooner the polyimide will degrade to the point where it fails to protect the
fused silica, and the shorter the overall mechanical lifetime of the column will
be.
...
hump'') topped by alkane peaks will be instantly recognizable as a representing
...
the blend were also separated on the SFC dimension \autocite{Venter1999}. This
...
method. But radiochemical methods requires specialized equipment and trained staff, whereas fuel
