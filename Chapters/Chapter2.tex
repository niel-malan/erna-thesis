...
method is usually called \keyword{supercritical fluid extraction} (SFE).
...
constant: and this temperature is known as the \keyword{boiling point}.
...
ionization detector (FID). The \keyword{flame ionization detector} was invented
...
current between the two electrodes with an \keyword{electrometer}.
...
second, chiral separation (on TLC) is said to be \keyword{orthogonal} to the
...
\keyword{heart-cutting}. In such a case a fraction of interest is collected from
the \keyword{first dimension} (\oneD) separation, and subjected to a
\keyword{second dimension} (\twoD) separation. For example, a fraction collected
