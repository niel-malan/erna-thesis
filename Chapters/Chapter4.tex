...
\label{sec:Part2}% Chapter 4it

\begin{savequote}[\quotewidth]
\SI{304.1}{\kelvin}, \SI{7.38}{\mega\pascal}, \SI{0.469}{\gram\per\cubic\centi\metre}
\qauthor{The critical temperature, pressure, and density of carbon dioxide.}
\end{savequote}

\chapter{Supercritical Fluid Chromatography} % Main chapter title
...
This pump had its control electronics removed and was controlled from a
computer. The Varian 8500 pump is driven by a stepper motor. This kind of motor
turns in discrete steps instead of at a constant rate. It is driven by pulses of
...
relatively simple to control the speed of the motor from a computer. The Varian
8500 pump has a built-in pressure transducer that provides an electronic signal
proportional to the pressure at the pump outlet.
...
However, filling a piston pump with liquid carbon dioxide can be a challenge.
...
chromatography. Instead, proxies for distance such as \keyword{retention
volumes} or \keyword{retention times} are used.
...
The six-port valve with an external sample loop is an alternative injection
...
where \(R_s\) is the \keyword{peak resolution}, \(N\) is the \keyword{number of
...
This restrictor design is robust and fairly simple to manufacture. It was
...
The blockages did not have an origin in the modifier, because no modifiers was
used. The possibility of blockage by sample material was eliminated by using a
brand new column. The suspicion is that the blocking material might be remnants
of surfactants used in the synthesis of the silica gel stationary phase. These
...
chosen. The choice was a simple linear restrictor, and the heat of the hot GC
inlet was trusted to prevent discrimination. The final linear restrictor was a
\SI{800}{\milli\metre} long fused silica capillary with an internal diameter of
